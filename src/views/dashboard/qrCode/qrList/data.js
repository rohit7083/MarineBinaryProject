...
export const DeleteConfirmationModal = ({ isOpen, toggle, itemToDelete, onConfirmDelete }) => {
  return (
    <>
      {isOpen && (
        <div 
...
export const qrCodeColumns = ({ onViewDetails, onEdit, onDelete, onDownload, onViewQRCode }) => {
  return [
    ...
    // },
    {
      ...
      cell: row => (
        <Badge color={qrCodeTypeColors[row.qrCodeType?.toLowerCase()] || qrCodeTypeColors.default} pill>
          {row.qrCodeType}
        </Badge>
      )
    },
    {
      ...
      cell: row => <span>{row.amount ? formatCurrency(row.amount) : 'N/A'}</span>
    },
    {
      name: 'Amount Type',
      sortable: true,
      ...
      cell: row => <span className='text-capitalize'>{row.amountType || 'N/A'}</span>
    },
    {
      name: 'Actions',
      minWidth: '200px',
