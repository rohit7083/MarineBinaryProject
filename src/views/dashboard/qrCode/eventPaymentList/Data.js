...
export const qrCodeColumns = ({ onViewDetails, onEdit, onDownload }) => {
  return [
    ...
    },
    {
      ...
      cell: row => <span className='text-nowrap'>{formatDate(row.transactionDate)}</span>
    },
    {
      name: 'Customer Name',
      sortable: true,
      ...
      cell: row => <span className='fw-bold'>{row.customerName || 'N/A'}</span>
    },
    {
      name: 'Customer Phone',
      sortable: true,
    ...
    },
    {
      ...
      cell: row => <span className='text-primary fw-bold'>{row.eventName}</span>
    },
    {
      name: 'QR Code Type',
      sortable: true,
    ...
    },
    {
      ...
      cell: row => <span className='font-small-2'>{row.transactionId || 'N/A'}</span>
    },
   
    {
      ...
      cell: row => {
        const colorClass = paymentStatusColors[row.status?.toUpperCase()] || 'light-secondary'
        return <Badge color={colorClass} pill>{row.status?.toUpperCase() || 'UNKNOWN'}</Badge>
      }
    },
    {
      name: 'Actions',
