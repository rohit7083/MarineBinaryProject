...
const index = () => {
  ...
  async function fetchTableData() {
    try {
      setLoading(true);
      const { data } = await useJwt.getAllCoupons();
      const { content } = data;
  ...
  const handleEdit = (row) => {
    navigate("/create-coupon", { state: { row } });
  };
  ...
  const columns = [
...

    {
      ...
      sortable: true,
      // minWidth: "150px",
      selector: (row) => row.durationUnit,
    },
    {
      name: "DurationValue",
      sortable: true,
      // minWidth: "150px",
      selector: (row) => row.durationValue,
    },
          ...
          return MySwal.fire({
            ...
            if (result.value) {
              try {
                ...
                // Call delete API
                const response = await useJwt.deleteCoupon(uid?.uid);
