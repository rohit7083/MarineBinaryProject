...
const CustomLabel = ({ htmlFor }) => (
  <Label className="form-check-label" htmlFor={htmlFor}>
    <span className="switch-icon-left"><Check size={14} /></span>
    <span className="switch-icon-right"><X size={14} /></span>
  </Label>
);

const ClientDetaiils = ({ memberAppendData, setSelectedMember, setMemberAppendData ,setOpen,open,setValueParent}) => {
  ...
  const onSubmit = (data) => {
    ...
    setValueParent('selectedMember',{label:`${data.firstName} ${data.lastName}`,value:JSON.stringify(payload),...payload})
    ...
    <Fragment>
      ...
      >
        <Form onSubmit={handleSubmit(onSubmit)}>
          ...
          <Row>
            <Col md="6" className="mb-1">
              <Label for="emailId">Email<span className="text-danger">*</span></Label>
              <Controller
                name="emailId"
                rules={{
                  required: "Email is required",
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    ...
                    options={countryOptions}
                    onChange={(option) => field.onChange(option)}
                    value={countryOptions.find(option => option.value === field.value?.value)}
                  />
                )}
              />
