...
setTimeout(()=>{
  Navigate('/dashboard/slipmember_list')
      ...
      const errorMessage = error?.response?.data?.message || error?.message || 'Error submitting payment. Please try again.'
    } finally {
      setLoading(false)
    }
  }
          ...
          <Row className="mb-2">
            <Col sm="4">
              ...
              {errors.pin && errors.pin.some(err => err) && (
                <FormFeedback className="d-block">
                  {errors.pin.find(err => err)?.message || 'All pin digits are required'}
                </FormFeedback>
              )}
            </Col>
          </Row>
                ...
                <Controller
                  ...
                  rules={{
                    ...
                    validate: (value) => {
                      ...
                      if (value < currentYear) return 'Expiry Year cannot be in the past'
                      return true
                    }
                  }}
                  render={({ field }) => (
                ...
                <Controller
                  ...
                  rules={{
                    ...
                    validate: (value) => {
                      ...
                      if (value < 1 || value > 12) return 'Expiry Month must be between 1 and 12'
                      return true
                    }
                  }}
                  render={({ field }) => (
                    <Select
                      {...field}
                      options={availableMonths.length > 0 ? availableMonths : months}
                      ...
                      classNamePrefix="select"
                      isClearable
                      value={(availableMonths.length > 0 ? availableMonths : months).find(
