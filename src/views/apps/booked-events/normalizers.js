...
export function normalizeEvent(e) {
  ...
  return {
    ...
    start: combineDateTime(
      e?.eventStartDate || e?.checkInDate,
      e?.eventStartTime || "00:00:00"
    ),
    end: combineDateTime(
      e?.eventEndDate || e?.checkOutDate,
      e?.eventEndTime || "23:59:59"
    ),
    // uid: e.uid,
    allDay: false,
    eventUID: e?.eventUID,
...
export function normalizeCalendarResponse(api) {
  ...
  const events = (api.content.roomBookings || [])
    .filter((b) => b && b.eventName)
    .map((b) => normalizeEvent(b));

  const rooms = (api.content.roomBookings || [])
    .filter((r) => r && !r.eventName)
    .map((r) => normalizeRoomBooking(r));
  // const events = (api.content.events || []).map(e => normalizeEvent(e));
  // const rooms = (api.content.roomBookings || []).map((r) =>
  //   normalizeRoomBooking(r)
  // );
  return [...events, ...rooms];
}
